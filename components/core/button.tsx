...
const Button = ({ buttonText, buttonLink, secondary, className, newTab, isHomePage }: Props) => {
  return (
    <Link
      href={buttonLink ?? ''}
      ...
      className={`inline-block px-4 py-2 md:px-5 lg:px-6 xl:px-8 md:py-3 font-medium text-sm rounded-full cursor-pointer ${secondary ?
        ...
        : isHomePage ? 'bg-white text-black-0 border border-primary hover:bg-primary-light hover:text-white' :
